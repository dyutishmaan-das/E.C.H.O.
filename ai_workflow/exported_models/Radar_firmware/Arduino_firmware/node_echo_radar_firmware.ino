...
void setup() {
  Serial.begin(115200);
  Serial.setTimeout(
      50); // Important: Prevents Serial.read from freezing the loop
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (input.equalsIgnoreCase("CLEAR") || input.equalsIgnoreCase("ACK")) {
      ...
      for (int i = 0; i < NUMBER_OF_INPUTS; i++) {
        ...
        String valStr = (nextCom == -1) ? input.substring(startIdx)
                                        : input.substring(startIdx, nextCom);
  ...
  // GIVE THE USER A 5-SECOND WINDOW TO PASTE THE NEXT STRING WITHOUT RESETTING
  // THE STREAK
