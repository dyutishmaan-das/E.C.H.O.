...
const int MORSE_UNIT_MS = 120;
const int MORSE_STEPS = 18;
const unsigned int morseDurations[MORSE_STEPS] = {
    1 * MORSE_UNIT_MS, 1 * MORSE_UNIT_MS, 1 * MORSE_UNIT_MS, 1 * MORSE_UNIT_MS, 1 * MORSE_UNIT_MS, 3 * MORSE_UNIT_MS,
    3 * MORSE_UNIT_MS, 1 * MORSE_UNIT_MS, 3 * MORSE_UNIT_MS, 1 * MORSE_UNIT_MS, 3 * MORSE_UNIT_MS, 3 * MORSE_UNIT_MS,
    1 * MORSE_UNIT_MS, 1 * MORSE_UNIT_MS, 1 * MORSE_UNIT_MS, 1 * MORSE_UNIT_MS, 1 * MORSE_UNIT_MS, 7 * MORSE_UNIT_MS
};
const bool morseBuzzOn[MORSE_STEPS] = {
...
// Lightweight local TFLite wrapper to avoid AllOpsResolver RAM overhead on
// ESP8266.
namespace {
...
void updateIndicators(AlertMode mode) {
  const unsigned long now = millis();
  if (now - patternLastMs >= 250) {
    patternLastMs = now;
  ...
  if (mode == MODE_FRIENDLY) {
    // Both yellow pair and red/blue pair alternate.
...
void setup() {
  ...
  // Startup background calibration (no target in front of radar).
  // This adapts activity gates to local analog noise so idle scenes don't
  // trigger ENEMY.
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    // Stream each value directly into the model input to avoid a large RAM
    // buffer.
    for (int i = 0; i < NUMBER_OF_INPUTS; i++) {
      ...
      frameSum += sample;
      if (hasPrevSample)
        frameDiffSum += fabsf(sample - prevSample);
      prevSample = sample;
      hasPrevSample = true;
    ...
    while (Serial.available())
      Serial.read(); // Flush trailing buffer
  ...
  // Ignore idle/noise frames so static background cannot keep system stuck on
  // ENEMY.
  if (!testDataReceived && frameSpan < spanGate && frameDiff < diffGate) {
    enemyStreak = 0;
    currentMode = MODE_IDLE;
    updateIndicators(currentMode);
    Serial.printf("{\"node\":\"ECHO-01\",\"type\":\"None\",\"conf\":0.00,"
                  "\"status\":\"NORMAL\",\"span\":%.3f,\"diff\":%.4f}\n",
                  frameSpan, frameDiff);
    delay(50);
    return;
  }
  ...
  Serial.printf(
      "{\"node\":\"ECHO-01\",\"type\":\"%s\",\"conf\":%.2f,\"status\":\"%s\","
      "\"span\":%.3f,\"diff\":%.4f,\"streak\":%d}\n",
      labels[prediction], confidence, isEnemy ? "ENEMY" : "NORMAL", frameSpan,
      frameDiff, enemyStreak);
